...
public class BoardTestSuit {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        long workDays = project.getTaskLists().stream()
                .filter(inProgressTask::contains)
                .flatMap(taskList ->  taskList.getTasks().stream())
                .map(Task::getCreated)
                .mapToInt(t -> t.compareTo(LocalDate.now()))
                .sum();
        ...
        long average = workDays / quantityTasks;
        //Then
        Assert.assertEquals(-11, workDays);
        Assert.assertEquals(3, quantityTasks);
        Assert.assertEquals(-3, average);
